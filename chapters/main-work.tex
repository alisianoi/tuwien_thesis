...
\begin{tikzpicture}
    \begin{umlpackage}{lyra2}
      ...
      \end{umlpackage}
  \end{tikzpicture}
  \caption{Approximate Class Diagram for \texttt{lyra2-java} Package}
  \label{fig:uml}
  \end{figure}
...
The original Lyra2 reference implementation requires just one direction for rotations. Both types of rotations are helpful in lyra2-java because they allow to avoid a number of byte rotations related to endianness. For an example of this,  refer to figure \ref{fig:sponge-blake2b}. It shows the \texttt{SpongeBlake2b} implementation, where lines \(5, 8\), and \(11\) use a left rotation. The original Blake2b specification prescribes a right rotation for those steps. However, instead of calling \texttt{mem.flip} that would rotate its argument once and then actually rotating it with \texttt{Sponge.rotr64}, a single rotation in the other direction with \texttt{Sponge.rotl64} is performed.
