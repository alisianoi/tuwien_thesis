...
\emph{Conventional Passwords} are by far the most widespread method of user authentication. According to sources from \cite{ives:2004:domino}, a typical user has to manage roughly 15 services that require a password while actually using at most 5 actually unique passwords. This leads to significant password reuse \cite{gaw2006password, shay2010encountering}. This in turn means that the most protected account of the user becomes as secure as their least protected one. Accordingly, as the number of reuses of a single password grows, so does the probability of its compromise.

Advanced password guessing techniques provide interesting insight as well. The authors of \cite{dell:2010:password} show that when training a Markov model for password search on their datasets, the second best training set is the usernames (the best being, of course, the set of passwords themselves). They explain it by suggesting that the user ususally creates both the username and the password in short succession, thus applying similar techniques.
