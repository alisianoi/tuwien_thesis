...
\begin{itemize}
    ...
    \item Libraries that implement the protocol have had major security issues: 
  \end{itemize}

The provided list of compromised CAs if far from complete, it serves to illustrate the point of \emph{lack of trust} in the concept of CAs.

\subsection{Password Authenticated Key Exchange}
\label{subsec:pake}

\emph{Password Authenticated Key Exchange (PAKE)}, which is a family of methods that establish secure communications over an insecure channel while simultaneously denying the opportunity for an attacker to run an offline dictionary attack. Insight into particular PAKE algorithms and their properties is provided below.

\emph{Encrypted Key Exchange (EKE)} was the first method from the PAKE family \cite{bellovin1992encrypted, bellovin1993augmented}. It allows the communicating parties to establish a secure connection with the following properties:

\begin{enumerate}
    \item The (weak) conventional password is protected from the offline dictionary attack. Instead, the attacker has to recover a (much stronger) session key.
    \item \emph{Replay attacks} are not possible because the session key is generated by using both the (weak) conventional password as well as a strong random value.
    \item \emph{Man-in-the-middle attacks} are not possible due to the challenge-response nature of the EKE handshake.
  \end{enumerate}

The early first version of the protocol is outlined below. This is done for simplicity, later versions addressed the issues of password hashing for storage, server compromise, etc.
...
\emph{Password Authenticated Key Exchange by Juggling (J-PAKE)} is a more recent and advanced method from the PAKE family \cite{hao2010j}. Its usage has the following security benefits \cite{abdalla2015security}:
...
Apart from security features, there is another strong argument in favour of J-PAKE: it is not patent encumbered.
