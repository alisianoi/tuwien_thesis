...
\emph{Phishing Attacks} is yet another formidable type of an attack. A legitimate user is usually tricked into visiting a webpage which impersonates a typical webservice, like an e-mail provider or a social network. The user is then prompted to enter their authentication details. If the user does not recognize the attack in time their credentials will get compromised. Phishing attacks usually target a group of individuals. On the other hand, \emph{spear phishing attacks} is a term reserved for targeting specific individuals (journalists, politicians, high-profile executives, etc.) in a much more personal and detailed manner.
