...
\subsection{Secure Authentication Protocols}
...
\emph{Conventional Password} is by far the most widespread method of user authentication. Usually the user needs to go through a registration process with a service, coming up with a username and a password. However, the resulting passwords are often weak and users have trouble remembering them too.

\emph{Graphical Passwords} can generally be subdivided into two categories: recognition- and recall based. The recognition schemes vary widely, the main idea being that the user has to recognize correct images and perform an action. For example, clicking on a previously chosen pictures or clicking inside a convex hull created by them. The pictures can be of human faces (because they are arguably the easiest to remember) or following a specific theme (foods, landscapes, etc.). Such authentication schemes often require more time than the conventional approach. The recall-based schemes 
...
\emph{Biometric Authentication} in general is a promising technique which focuses on physical traits unique to every user. These include fingerprints, facial recognition, voice recognition, iris scanning and similar.
...
\subsection{Types of Authentication Attacks}
