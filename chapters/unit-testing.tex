...
\textsc{Boost.Test} provides all the common features of a unit-testing library: a developer can decouple the tests from the implementation, subdivide the tests into suits and automatically register them with the test runner. The registration can be accomplished a single \mintinline{cpp}{#include<...>} and a call to the \mintinline{cpp}{BOOST_AUTO_TEST_CASE} macro, as shown in \ref{figure:boost-auto-test-case}.
