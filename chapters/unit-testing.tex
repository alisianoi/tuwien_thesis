...
The data for parametrized tests is stored in \gls{yaml} files. Those files are produced by the \texttt{lyra2-c} project with the help of the Python build harness. Then they are consumed by the JUnit4 testing framework of the \texttt{lyra2-java} project. This mechanism allows to make sure that both projects compute the same hash values.
