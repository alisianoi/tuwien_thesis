...
The YAML format is also used to store default configuration for the Python test harness. It can be found on the \texttt{harness} branch of the forked reference repository \cite{github:2017:lyra-copy} in the \mintinline{shell}{Lyra2/src/tests/harness.yml}. The primary parameters are \mintinline{shell}{build_path} and \mintinline{shell}{makefile_path}. The first one defines the location for the compiled Lyra2 executables and the second one points to the original \mintinline{shell}{Makefile}.
